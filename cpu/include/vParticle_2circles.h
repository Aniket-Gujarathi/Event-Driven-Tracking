...
public:
    ...
    void initialiseState(double x, double y, double r, double r2);
    void randomise(int x, int y, int r, int r2);

    void resetWeight(double value);
    void resetRadius(double value);
    void resetArea();
    void setContraints(int minx, int maxx, int miny, int maxy, int minr, int maxr, int minr2, int maxr2);
    ...
    inline void incrementalLikelihood(int vx, int vy, int n)
    {
        ...
        // smaller circle
        double sqrd2 = sqrt(pow(dx, 2.0) + pow(dy, 2.0)) - r2;
        ...
        double cval = NULL;
        if(-2.0 <= sqrd <= 2.0 || -1.0 <= sqrd2 <= 1.0)
            cval = 1.0;
        else if(-3.0 <= sqrd < -2.0 || -2.0 <= sqrd2 < -1.0)
...
public:
    ...
    void setSeed(int x, int y, int r = 0, int r2 = 0);
    void resetToSeed();
    ...
    void extractTargetPosition(double &x, double &y, double &r, double &r2);
