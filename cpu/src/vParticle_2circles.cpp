...
void vParticle::initialiseState(double x, double y, double r, double r2)
{
    this->x = x;
    this->y = y;
    this->r = r;
...
void vParticle::randomise(int x, int y, int r, int r2)
{
    initialiseState(rand()%x, rand()%y, rand()%r, rand()%r2);
}
...
//    r = generateGaussianNoise(r, sigma * 0.2);
    ...
    r2 = generateUniformNoise(r2, sigma * 0.2);

    if(constrain) checkConstraints();
}

void vParticle::setContraints(int minx, int maxx, int miny, int maxy, int minr, int maxr, int minr2, int maxr2)
{
    this->minx = minx;
    this->maxx = maxx;
    this->miny = miny;
                                 ...
                                 double randoms, double negativeBias)
{
    ...
    rbound_min = res.width/32;
    rbound_max = res.width/21;
    rbound2_min = res.width/128;
    rbound2_max = res.width/43;
    ...
    p.setContraints(0, res.width, 0, res.height, rbound_min, rbound_max, rbound2_min, rbound2_max);
...
void vParticlefilter::setSeed(int x, int y, int r, int r2)
{
    seedx = x; seedy = y; seedr = r; seedr2 = r2;
}
...
void vParticlefilter::resetToSeed()
{
    if (seedr){
        for(int i = 0; i < nparticles; i++) {
            ps[i].initialiseState(seedx, seedy, seedr, seedr2);
        }
    }
    else{
        for(int i = 0; i < nparticles; i++) {
            return ps[i].initialiseState(180, 160, 30, 10);
        }
    }
}
...
void vParticlefilter::extractTargetPosition(double &x, double &y, double &r, double &r2)
{
    x = 0; y = 0; r = 0; r2 = 0;
...
void vParticlefilter::performResample()
{
    if(!adaptive || pwsumsq * nparticles > 2.0) {
        ...
        for(int i = 0; i < nparticles; i++) {
            ...
            if(rn > 1.0)
                ps[i].randomise(res.width, res.height, rbound_max, rbound2_max);
            else {
                int j = 0;
