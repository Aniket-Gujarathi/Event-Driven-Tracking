...
{
    negativeScaler = negativeBias * angbuckets / (M_PI * ((a + b) / 2) * ((a + b) / 2));
}
...
double vParticle::findIntersection(int &vx, int &vy, double &x, double &y, double &a, double &b){
    double x_ell, y_ell;
    if (x == vx){
        return NULL, NULL;
    }
    ...
    if (vy >= y){
        y_ell = y + (m*a*b) / (sqrt(b*b + m*m*a*a));
    }
    else{
        y_ell = y - (m*a*b) / (sqrt(b*b + m*m*a*a));
    }
                                 ...
                                 double randoms, double negativeBias)
{
    ...
    rbound_min = res.width / 42;
    rbound_max = res.width / 18;
    ...
    p.setContraints(0, res.width, 0, res.height, rbound_min, rbound_max, rbound_min, rbound_max);
...
void vParticlefilter::performResample()
{
    if(!adaptive || pwsumsq * nparticles > 2.0) {
        ...
        for(int i = 0; i < nparticles; i++) {
            ...
            if(rn > 1.0)
                ps[i].randomise(res.width, res.height, rbound_max, rbound_max);
            else {
                int j = 0;
