...
#include "vControlLoopDelay_2circles.h"
...
int main(int argc, char * argv[])
{
    ...
    rf.setDefaultConfigFile( "vParticleFilterTracker_2circles.ini" );
...
void delayControl::performReset(int x, int y, int r, int r2)
{
    if(x > 0)
        vpf.setSeed(x, y, r, r2);
    vpf.resetToSeed();
    input_port.resume();
}
...
void delayControl::run()
{
    ...
    vpf.extractTargetPosition(avgx, avgy, avgr, avgr2);
    ...
    while(true) {
        ...
        dx = avgx, dy = avgy, dr = avgr; dr2 = avgr2;
        vpf.extractTargetPosition(avgx, avgy, avgr, avgr2);
        dx = avgx - dx; dy = avgy - dy; dr = avgr - dr; dr2 = avgr2 - dr2;
        ...
        if(debugPort.getOutputCount()) {
            ...
            if(panelnumber < NOFPANELS) {
                ...
                if (px1<avgx<px2 && py1<avgy<py2){
                    ...
                    cv::circle(cvImg, Point(avgx, avgy), avgr2, (255, 255, 255), 2);
                }
