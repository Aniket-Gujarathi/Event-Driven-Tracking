...
def drawPara(image, x, y, theta, c, norm_score):
    ...
    # drawPara(image, x_c[i], y_c[i], theta[i], c[i], norm_score[i])
    # image = cv2.circle(image, (x_c[i], y_c[i]), 0, (0, 255, int(norm_score[i])), 5)
    cv2.polylines(image, np.array([pts]), False, [0, 0, int(norm_score)], 1)

def draw(image, filename):
    x_c, y_c, theta, c, score, likelihood = read_data(filename)
    max_score = np.amax(score)
    min_score = np.amin(score)
    norm_score = normalize(score, max_score, min_score)
    print(np.mean(x_c), np.std(x_c))
    print(np.mean(y_c), np.std(y_c))
